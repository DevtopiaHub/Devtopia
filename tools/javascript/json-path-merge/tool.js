...
if (!input.target || typeof input.target !== 'object') { console.log(JSON.stringify({ ok: false, error: 'Missing field: target (object)' })); process.exit(1); }
...
try {
  const value = devtopiaRun('json-path-get', { obj: input.target, path: input.path });
  ...
  console.log(JSON.stringify({ ok: true, path: input.path, extracted: value.value, merged: merged.result, steps: ["json-path-get","json-merge-deep"] }));
