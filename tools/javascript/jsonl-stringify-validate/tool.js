...
try {
  ...
  const stringified = devtopiaRun('jsonl-stringify', { array: input.array });
  if (!stringified.ok) { console.log(JSON.stringify({ ok: false, error: stringified.error })); process.exit(1); }
  console.log(JSON.stringify({ ok: true, jsonl: stringified.jsonl, lines: input.array.length, steps: ["schema-required","jsonl-stringify"] }));
